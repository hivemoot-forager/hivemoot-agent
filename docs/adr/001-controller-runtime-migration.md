...
| Docker API | `moby/moby/client` | `docker-py` | `bollard` crate |
...
| Agent-maintainable | ✅ with Go toolchain | ✅ with Python | ⚠️ steep learning curve |
| Ecosystem fit | CNCF-standard (Docker, K8s, GH CLI) | Strong scripting/ML | Over-engineered for I/O-bound work |

### Why Go specifically

**moby/moby/client** maps directly to what `spawn_worker()` does via subprocess:
...
±2% over 48 hours. Rollback: `CONTROLLER_BINARY=shell` env var restores shell path.
