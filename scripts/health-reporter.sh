...
_build_health_payload() {
  ...
  if [ -n "$run_summary_text" ]; then
    jq_args+=(--arg run_summary "$run_summary_text")
...
#   run_summary          — plain-text run summary string (optional, max 1500 bytes)
