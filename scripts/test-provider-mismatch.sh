#!/usr/bin/env bash
set -euo pipefail

fail() {
  echo "FAIL: $*" >&2
  exit 1
}

assert_fails_with() {
  local expected="$1"
  shift

  local stderr_file
  stderr_file="$(mktemp)"

  if "$@" > /dev/null 2> "$stderr_file"; then
    rm -f "$stderr_file"
    fail "command succeeded unexpectedly: $*"
  fi

  if ! grep -Fq "$expected" "$stderr_file"; then
    echo "Expected stderr to contain:" >&2
    echo "  $expected" >&2
    echo "Actual stderr:" >&2
    sed 's/^/  /' "$stderr_file" >&2
    rm -f "$stderr_file"
    fail "stderr mismatch for: $*"
  fi

  rm -f "$stderr_file"
}

assert_succeeds() {
  local description="$1"
  shift

  local stderr_file
  stderr_file="$(mktemp)"

  if ! "$@" > /dev/null 2> "$stderr_file"; then
    echo "Unexpected failure for: $description" >&2
    sed 's/^/  /' "$stderr_file" >&2
    rm -f "$stderr_file"
    fail "command failed unexpectedly"
  fi

  rm -f "$stderr_file"
}

tmp_home="$(mktemp -d)"
cleanup() { rm -rf "$tmp_home"; }
trap cleanup EXIT

echo "Running provider mismatch detection checks"

# Mismatch: image built for codex but agent requests claude
assert_fails_with \
  "Provider mismatch: image built with PROVIDER=codex but AGENT_PROVIDER=claude." \
  env HOME="$tmp_home" \
      DOCKER_PROVIDER=codex \
      AGENT_PROVIDER=claude \
      RUN_MODE=once \
      bash scripts/entrypoint.sh

# Mismatch: image built for claude but agent requests gemini
assert_fails_with \
  "Provider mismatch: image built with PROVIDER=claude but AGENT_PROVIDER=gemini." \
  env HOME="$tmp_home" \
      DOCKER_PROVIDER=claude \
      AGENT_PROVIDER=gemini \
      RUN_MODE=once \
      bash scripts/entrypoint.sh

# No mismatch: DOCKER_PROVIDER=all passes through regardless of AGENT_PROVIDER
# (all-provider image supports any provider — use invalid RUN_MODE to stop early)
assert_fails_with \
  "Invalid RUN_MODE" \
  env HOME="$tmp_home" \
      DOCKER_PROVIDER=all \
      AGENT_PROVIDER=claude \
      RUN_MODE=invalid \
      bash scripts/entrypoint.sh

# No mismatch: DOCKER_PROVIDER matches AGENT_PROVIDER
assert_fails_with \
  "Invalid RUN_MODE" \
  env HOME="$tmp_home" \
      DOCKER_PROVIDER=claude \
      AGENT_PROVIDER=claude \
      RUN_MODE=invalid \
      bash scripts/entrypoint.sh

# No mismatch: DOCKER_PROVIDER unset (defaults to all, passes through)
assert_fails_with \
  "Invalid RUN_MODE" \
  env HOME="$tmp_home" \
      AGENT_PROVIDER=codex \
      RUN_MODE=invalid \
      bash scripts/entrypoint.sh

echo "PASS: provider mismatch detection checks"
